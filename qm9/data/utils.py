...
def initialize_datasets(args, datadir, dataset, subset=None, splits=None,
    ...
    for split, datafile in datafiles.items():
        with np.load(datafile) as f:                
            ...
            # datasets[split] = {key: val for key, val in f.items()}
    # for key in datasets['train']:
    #     print('The shape', key ,': ', datasets['train'][key].shape)
    ...
    if remove_h:
        ...
        for key, dataset in datasets.items():
            ...
            # print('new_datasets',datasets['train']['charges'].shape)

    # Get a list of all species across the entire dataset
    all_species, all_species_subgraphs = _get_species(datasets, ignore_check=False)

    # Now initialize MolecularDataset based upon loaded data
    datasets = {split: ProcessedDataset(data, num_pts=num_pts.get(
        split, -1), included_species=all_species, all_species_subgraphs =all_species_subgraphs, subtract_thermo=subtract_thermo) for split, data in datasets.items()}
    ...
    # print("after ProcessedDataset one_hot: ", x['one_hot'].shape)
    # print("after ProcessedDataset charges: ", x['charges'].shape)
...
def _get_species(datasets, ignore_check=False):
    ...
    included_species_subgraphs = torch.cat([(dataset['subgraph_charges']).unique()
                        for dataset in datasets.values()]).unique(sorted=True)
    all_species_subgraphs = included_species_subgraphs[1:]
    # included_species = torch.cat([(dataset['charges']).unique()
    #                             for dataset in datasets.values()]).unique(sorted=True)
    # included_species = included_species_subgraphs[1:]
    ...
    return all_species, all_species_subgraphs

...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def kl_prior(self, xh, property_val, node_mask):
        ...
        # normalize [1,2]
        property_val = property_val / (max(property_val) - min(property_val)) + 1

        # Compute means.
        mu_T = alpha_T * xh
        mu_T_x, mu_T_h = mu_T[:, :, :self.n_dims], mu_T[:, :, self.n_dims:]

        mu_T_x = (mu_T_x * node_mask).sum(1)/node_mask.sum(1)
        mu_T_h = (mu_T * node_mask).sum(1)/node_mask.sum(1)
        ...
        property_val_h = property_val.unsqueeze(1).expand(mu_T_h.shape[0], mu_T_h.shape[1])
        # Compute KL for h-part.
        zeros, ones = torch.zeros_like(mu_T_h), torch.ones_like(sigma_T_h)
        if (self.unify_condition):
            kl_distance_h = gaussian_KL(mu_T_h, sigma_T_h, property_val_h, ones, node_mask)
        else:
        ...
        property_val_x = property_val.unsqueeze(1).expand(mu_T_x.shape[0], mu_T_x.shape[1])
        if (self.unify_condition):
            kl_distance_x = gaussian_KL_for_dimension(mu_T_x, sigma_T_x, property_val_x, ones, d=subspace_d)
        else:
    ...
    def compute_loss(self, x, h, property_val, node_mask, edge_mask, context, t0_always):
        ...
        kl_prior = self.kl_prior(xh, property_val, node_mask)
...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def compute_sub_reconstruction_error(self, subgraph_h_rec, subgraph_h):
        """Computes subgraph reconstruction error."""
        subgraph_h = torch.cat([subgraph_h['categorical'], subgraph_h['integer']], dim=2)
        bs, n_nodes, _ = subgraph_h.shape
        subgraph_h = subgraph_h.reshape(bs * n_nodes, -1)
        # print('subgraph_h_rec: ', subgraph_h_rec.shape)
        # print('subgraph_h: ', subgraph_h.shape)
        # Error on positions.
        # Error on classes.
        h_cat_rec = subgraph_h_rec[:, :-1]
        h_cat = subgraph_h[:, :-1]
        # h_cat_rec = h_cat_rec.reshape(bs * n_nodes, -1)
        # h_cat = h_cat.reshape(bs * n_nodes, -1)
        error_h_cat = F.cross_entropy(h_cat_rec, h_cat.argmax(dim=1), reduction='none')
        # print('error_h_cat: ', error_h_cat.shape)
        error_h_cat = error_h_cat.reshape(bs, n_nodes, 1)
        error_h_cat = sum_except_batch(error_h_cat)

        # Error on charges.
        if self.include_charges:
            h_int_rec = subgraph_h_rec[:, -1]
            h_int = subgraph_h[:, -1]
            h_int_rec = h_int_rec.reshape(bs, n_nodes, 1)
            h_int = h_int.reshape(bs, n_nodes, 1)
            error_h_int = sum_except_batch((h_int_rec - h_int) ** 2)
        else:
            error_h_int = 0.

        # print('The shape of x_rec is ', x_rec.shape) # 3
        # print('The shape of h_cat_rec is ', h_cat_rec.shape) # 24
        # print('The shape of h_int_rec is ', h_int_rec.shape) # 1
        error = error_h_cat + error_h_int

        if self.training:
            denom = (self.n_dims + self.in_node_nf) * subgraph_h.shape[1]
            error = error / denom
    ...
    def encode(self, x, h,subgraph_h, subgraphs, node_mask=None, edge_mask=None, context=None):
        """Computes q(z|x)."""
        ...
        subgraph_h = torch.cat([subgraph_h['categorical'], subgraph_h['integer']], dim=2)
        # print('===========================')
        # print(subgraph_h.shape)
        # print('===========================')

        # print(h['categorical'].shape)
        # print(h['integer'].shape)
        diffusion_utils.assert_mean_zero_with_mask(xh[:, :, :self.n_dims], node_mask)

        # Encoder output.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma, subgraph_h_final = self.encoder._forward(xh,subgraph_h, subgraphs, node_mask, edge_mask, context)
        # unique_elements, indices = torch.unique(subgraphs, return_inverse=True)

        # z_x_mu_sub = torch.zeros_like(z_x_mu, device=z_x_mu.device)
        # z_h_mu_sub = torch.zeros_like(z_h_mu, device=z_h_mu.device)
        # subgraph_mask = torch.zeros_like(node_mask, device=node_mask.device)
        # n = z_x_mu.shape[1]
        # for i in range(z_x_mu.shape[0]):
        #     _, indice = torch.unique(subgraphs[i], return_inverse=True)
        #     z_x_mu_i = z_x_mu[i]
        #     z_h_mu_i = z_h_mu[i]
        #     # num_nodes = int((indice>0).sum())
        #     # indice = indice[indice>0] - 1
        #     z_x_mu_sub_i = global_mean_pool(z_x_mu_i, indice)
        #     z_h_mu_sub_i = global_mean_pool(z_h_mu_i, indice)
        #     pad_num = n - z_h_mu_sub_i.shape[0]
        #     z_x_mu_sub[i] = torch.cat((z_x_mu_sub_i[1:], torch.zeros([pad_num+1, 3], device=z_x_mu.device)))
        #     z_h_mu_sub[i] = torch.cat((z_h_mu_sub_i[1:], torch.zeros([pad_num+1, 1], device=z_x_mu.device)))
        ...
        return z_x_mu, sigma_0_x, z_h_mu, sigma_0_h, subgraph_h_final
    
    def decode(self, z_xh, node_mask=None, edge_mask=None, context=None):
        """Computes p(x|z)."""

        # Decoder output (reconstruction).
        x_recon, h_recon, subgraph_h_recon= self.decoder._forward(z_xh, subgraph_h_final, node_mask, edge_mask, context)
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def forward(self, x, h, subgraph_h, subgraphs, property_val, node_mask=None, edge_mask=None, context=None):
        """
        Computes the loss (type l2 or NLL) if training. And if eval then always computes NLL.
        """
        # Encode data to latent space.
        z_x_mu, z_x_sigma, z_h_mu, z_h_sigma, subgraph_h_final = self.vae.encode(x, h,subgraph_h, subgraphs, node_mask, edge_mask, context)
        ...
        subgraph_h_final = subgraph_h_final.detach()  # Always keep the encoder fixed.
        diffusion_utils.assert_correctly_masked(z_xh, node_mask)
        diffusion_utils.assert_correctly_masked(subgraph_h_final, node_mask)
        # Compute reconstruction loss.
        if self.trainable_ae:
            xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
            # Decoder output (reconstruction).
            x_recon, h_recon, subgraph_h_recon = self.vae.decoder._forward(z_xh, subgraph_h_final, node_mask, edge_mask, context)
            xh_rec = torch.cat([x_recon, h_recon], dim=2)
            loss_recon = self.vae.compute_reconstruction_error(xh_rec, xh)
            loss_recon += self.vae.compute_sub_reconstruction_error(subgraph_h_recon, subgraph_h)
        ...
        if self.training:
            # Only 1 forward pass when t0_always is False.
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, property_val, node_mask, edge_mask, context, t0_always=False)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, property_val, node_mask, edge_mask, context, t0_always=True)

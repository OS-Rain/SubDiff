...
def count_elements(numbers):
    count_dict = {}  # 创建一个空字典用于存储结果
    
    for num in numbers:
        if num in count_dict:
            count_dict[num] += 1  # 如果数字已经在字典中，计数加1
        else:
            count_dict[num] = 1  # 如果数字不在字典中，将其加入字典，并计数为1
...
dataset_info['in_node_nf'] = dataloaders['train'].dataset.data['one_hot'].shape[2]
dataset_info['in_subgraph_nf'] = dataloaders['train'].dataset.data['subgraph_one_hot'].shape[2]
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        train_epoch(args=args, loader=dataloaders['train'], epoch=epoch, model=model, model_dp=model_dp,
                    ...
                    gradnorm_queue=gradnorm_queue, optim=optim, prop_dist=prop_dist, property=property)
        ...
        if epoch % args.test_epochs == 0:
            ...
            nll_val = test(args=args, loader=dataloaders['valid'], epoch=epoch, eval_model=model_ema_dp,
                           ...
                           property_norms=property_norms, property=property)
            nll_test = test(args=args, loader=dataloaders['test'], epoch=epoch, eval_model=model_ema_dp,
                            partition='Test', device=device, dtype=dtype,
                            nodes_dist=nodes_dist, property_norms=property_norms, property=property)

qm9_with_h = {
    ...
    'subgraph_encoder': {'H': 1, 'C': 2, 'N': 3, 'O': 4, 'F': 5},
    'subgraph_matcher': {1:'H', 2:'C', 3:'N', 4:'O' ,5:'F'},
...
def get_dataset_info(dataset_name, remove_h):
    if dataset_name == 'qm9':
        if not remove_h:
            return new_add_subgraph_dataset_info(qm9_with_h)
        else:
...
import pickle
def new_add_subgraph_dataset_info(qm9_with_h):
    with open("qm9/temp/qm9/dataset_info.pkl", "rb") as file:
        data = pickle.load(file)

    qm9_with_h['num_subgraphs'] = data['num_subgraphs']
    qm9_with_h['num_nodes_in_subgraph'] = data['num_subgraphs']
    qm9_with_h['subgraph_matcher'] = data['subgraph_macher']
    qm9_with_h['subgraph_decoder'] = data['subgraph_decoder']
    # dic_encoder = qm9_with_h['subgraph_matcher']
    # dic_decoder = qm9_with_h['subgraph_decoder']
    # num_atoms =  len(qm9_with_h['subgraph_encoder']) + 1
    # file_path = "preprocess/merging_operation.txt"  
    # motif_smiles_list = []
    ...
    #     dic_encoder.update({i+num_atoms : motif})    
    #     dic_decoder.append(motif)  
    # qm9_with_h['subgraph_matcher'] = dic_encoder
    # qm9_with_h['subgraph_decoder'] = dic_decoder

    return qm9_with_h

def add_subgraph_dataset_info(qm9_with_h):
    dic_encoder = qm9_with_h['subgraph_matcher']
    dic_decoder = qm9_with_h['subgraph_decoder']
    num_atoms =  len(qm9_with_h['subgraph_encoder']) + 1
    file_path = "preprocess/merging_operation.txt"  
